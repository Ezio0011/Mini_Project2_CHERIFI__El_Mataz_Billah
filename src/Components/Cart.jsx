...
const Cart = () => {
  ...
  if(carts.length === 0) {
    return <div className='cont h-[55vh] flex justify-center items-center text-4xl '>Cart is Empty</div>
  }

  return (
    <div className="cont mb-100 p-10 mx-auto ">
      <div className="flex shadow-md my-5">
                ...
                <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                  <div className="flex w-2/5">
                    ...
                    <div className="flex flex-col justify-between ml-4 flex-grow">
                      ...
                      <span className="text-blue-500 text-xs capitalize">{cart?.category}</span>
                      <div className="font-semibold hover:text-red-700 text-red-500 text-xs cursor-pointer" onClick={() => removeProduct(cart?.id)}><i class="fa-solid fa-trash"></i> Remove</div>
                    </div>
                  </div>
        ...
        <div id="summary" className="w-1/4 px-8 py-10">
          ...
          <div className="border-t mt-8">
            ...
            <button  onClick={showPopup} className="bg-indigo-500 font-semibold hover:bg-indigo-600 py-3 text-sm text-white uppercase w-full">Checkout</button>
          </div>
        </div>
      ...
      {isPopupVisible && (
        <div className="overlay">
          <div className="popup">
            ...
            <div>
              <p>You need to sign in.</p></div>
              <div>
              <Link to="/signin"> <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"> Sign In</button></Link>
              <button  className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" onClick={hidePopup}>Close</button>
              </div>
            </div>
          </div>
        </div>
